	...
	<div class="offlineThree">
		<ul class="offlineThree-main">
			...
			<li>
				<fieldset>
					<legend>搜索结果</legend>
					<el-pagination class="offline3_pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-size="pageAmount" layout=" total, prev, pager, next,jumper " :total="currentPageLength" data-mode="num"> </el-pagination>
					<el-table class="offline3_table" :data="peopleList" height="100%" white-space:nowrap :row-class-name="tableRowClassName" v-loading="staffBoolean">
						<el-table-column label="姓名" width="150" prop="name"> </el-table-column>
						<el-table-column label="性别" width="150" prop="sex"> </el-table-column>
						<el-table-column label="工号" width="150" prop="jobNumber"> </el-table-column>
						<el-table-column label="办公电话" width="230" prop="officePhone"> </el-table-column>
						<el-table-column label="移动电话" width="230" prop="mobilePhone"> </el-table-column>
						<el-table-column label="公司" width="300" prop="company"> </el-table-column>
						<el-table-column label="部门" width="210" prop="department"> </el-table-column>
						<el-table-column label="位置" width="210" prop="position"> </el-table-column>
						<el-table-column fixed="right" label="选择被访人" align="center" width="180">
							<template slot-scope="scope">
								<el-button v-if="!scope.row.isEgdit" icon="el-icon-check" type="primary" size="small" @click="handle(scope.row, scope.$index)">选择被访人</el-button>
							</template>
						</el-table-column>
					</el-table>
				</fieldset>
	...
	methods: {
		...
		async queryStaff() {
			...
			if (res.success) {
				...
				this.$message({ type: "success", message: `查询成功，为您查到${this.peopleList.length}条数据`  });
				this.getTableData();
			} else {
