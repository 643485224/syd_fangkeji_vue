<template>
	<div class="onsiteFive">
		<div class="onsiteFive-div2" v-if="disposesForm.miscellaneous.miscellaneous_printer">
			<div>
				<img :src="defaultPhoto" alt="" />
				<p>{{ messText }}</p>
			</div>
		</div>
		<div class="onsiteFive-div2" v-else>
		...
		<div @click="goHome()" class="onsite5_home">
...
export default {
	...
	created() {
		console.log("onsite4", this.$route.params.value);
			...
			APP.addVisitor(this.httpform, info)
				.then(async (resp) => {
					...
					if (resp.data.code !== 200) {
						...
						// this.$message({ type: "warning", message: "登记失败", duration: 4000 });
						return;
					}
	...
	computed: {
		...mapState(["registrationInformation", "disposesForm","httpform"]),
	},
	beforeDestroy() {
		clearInterval(this.timer);
		this.timer = null;
